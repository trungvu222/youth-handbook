...
const deleteUser = async (req, res, next) => {
  console.log('=== SOFT DELETE USER CALLED ===');
  try {
    const { id } = req.params;
    const currentUser = req.user;
    
    console.log('[Soft Delete] User ID:', id);
    console.log('[Soft Delete] Requested by:', currentUser?.email);
    ...
    await prisma.user.update({
      ...
      data: { 
        isActive: false,
        // Anonymize with unique timestamp to avoid email conflicts
    ...
    console.log('[Delete User] Successfully deactivated user:', id);
    ...
    res.status(500).json({
      success: false,
      error: 'Không thể xóa đoàn viên. Vui lòng thử lại sau.',
...
// Soft delete solution - 2026-01-27 v3
