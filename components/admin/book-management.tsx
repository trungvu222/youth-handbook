          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{stats?.totalBooks || books.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{stats?.currentlyBorrowed || 0}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Tên sách</th>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Mã QR</th>
                  ...
                  <tbody className="divide-y">
                    {paginatedBooks.map((book, index) => (
                      <tr key={book.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm">{(currentPage - 1) * ITEMS_PER_PAGE + index + 1}</td>
                        ...
                        <td className="px-4 py-3">
                          <Badge variant={book.isBorrowed ? 'destructive' : 'secondary'}>
                            {book.isBorrowed ? 'Đang mượn' : 'Sẵn sàng'}
                          </Badge>
                        </td>
                        <td className="px-4 py-3">
                          <Button variant="ghost" size="sm" onClick={() => openQRDialog(book)}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => { setSelectedBook(book); setShowDeleteDialog(true) }}>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-500">
                Hiển thị {(currentPage - 1) * ITEMS_PER_PAGE + 1}-{Math.min(currentPage * ITEMS_PER_PAGE, filteredBooks.length)} / {filteredBooks.length}
              </span>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">STT</th>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Người mượn</th>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Tên sách</th>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Tác giả</th>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">Nhà xuất bản</th>
                    ...
                    {filteredBorrowings.map((b, index) => (
                      <tr key={b.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          ...
                          <p className="text-xs text-gray-500">{b.borrowerUnit}</p>
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm">
                          {b.returnedAt ? (
                            ...
                            <Badge variant="outline" className="text-orange-600 border-orange-300">Chưa trả</Badge>
                          )}
                        </td>
                      </tr>
                    ))}
      ...
      <Dialog open={showQRDialog} onOpenChange={setShowQRDialog}>
        <DialogContent className="max-w-sm text-center">
          ...
          <DialogFooter className="flex justify-center gap-2">
            <Button variant="outline" onClick={() => setShowQRDialog(false)}>Đóng</Button>
